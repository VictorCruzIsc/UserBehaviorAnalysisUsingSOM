...
public class Dataset {
	...
	public static List<List<Package>> getRawPackages(String idUser){
		...
		List<List<Package>> basePackages = new ArrayList<>();
		...
		for(String dataSetFolderName : datasetFolderNames){
			...
			if(dataSetFlowsFileNames.size() > 0){
				List<Package> dataSetPackages = new ArrayList<>();
				
				for(String flowFileName : dataSetFlowsFileNames){
					List<String> fileContents = Utils.readFileContent(flowFileName);
					
					if(fileContents == null){
						Utils.logError("getRawPackages(" + idUser + ") fileContents is null, path: " +
								dataSetFolderName + "/" + flowFileName);
						continue;
					}
					
					//Utils.logInfo(fileName + " total lines:" + fileContents.size());
					for(String rawCapture : fileContents){
						Package currentPackage = new Package(rawCapture);
						if(!currentPackage.isError()){
							dataSetPackages.add(currentPackage);
						}else{
							Utils.logError("getRawPackages(" + idUser + ") in " + rawCapture);
						}
					}
				}
				
				basePackages.add(dataSetPackages);
			}else{
				Utils.logError("getRawPackages(" + idUser + ") fileNames size() is 0 path: " +
						dataSetFolderName);
				return null;
			}
		}
